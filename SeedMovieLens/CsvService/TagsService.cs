...
namespace SeedMovieLens.CsvService
{
    public class TagsService
    {
        ...
        public void ReadCsvFile(string location, string connectionString, int threads)
        {
            ...
            try
            {
                ...
                Logger.Info($"Inserting {countRecords} tags using {threads} threads");
        ...
        private void StartThread(string location, DbContext context, int skip, int take)
        {
            ...
            try
            {
                ...
                csv.Configuration.RegisterClassMap<TagMap>();
                int count = 0;
                foreach (var r in csv.GetRecords<Tags>().Skip(skip).Take(take))
                {
                    context.Add(r);
                    if (count == 1000)
                    {
        ...
        {
            foreach (var item in context.ChangeTracker.Entries<Tags>())
            {
    ...
    public sealed class TagMap : ClassMap<Tags>
    {
